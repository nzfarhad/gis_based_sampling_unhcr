...
filter_settlment <- function(settlements, settlements_boundry, af_buildings, raster1, my_id){
  ...
  settlements <- settlements %>% filter(Ref_ID %in% my_id)
  settlements_boundry <- settlements_boundry %>% 
    filter(Ref_ID %in% my_id)
...
# filter_settlment_32642 <- function(settlements, settlements_boundry, af_buildings, raster1, my_id){
#   
#   settlements <- settlements %>% filter(Ref_ID %in% my_id)
#   settlements_boundry <- settlements_boundry %>% 
#     filter(Ref_ID %in% my_id)
...
#   
#   af_buildings_filtered <- af_buildings %>% 
#     st_filter(settlements_boundry_32642, .predicate = st_intersects)
#   
#   # settlements_boundry_4326 <- settlements_boundry_32642 %>% st_transform(crs = 4326)
#   
#   # settlements_boundry_4326 <- settlements_boundry_4326 %>% st_transform(crs = 4326)
#   cropped_raster <- raster::crop(raster1, extent(settlements_boundry_32642))
#   masked_raster <- raster::mask(cropped_raster, settlements_boundry_32642)
#   
#   output_list <- list(settlements_filt = settlements,
#                       settlements_boundry_filt = settlements_boundry_32642,
#                       af_buildings_filt = af_buildings_filtered,
#                       population_raster = masked_raster
#   )
#   return(output_list)
...
# Create Grid - crs Web Mercator (EPSG:3857)
